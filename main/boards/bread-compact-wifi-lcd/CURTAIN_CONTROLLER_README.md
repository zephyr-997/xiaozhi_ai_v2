...
### 系统配置
- **FreeRTOS Tick**: 1000 Hz（1 ms 精度）手动修改
  - 系统时钟频率配置为 1000Hz，提供 1ms 级别的时间精度
  - 确保步进电机每步 2ms 延时的准确性
  - 影响 `vTaskDelay(pdMS_TO_TICKS(CURTAIN_SPEED_MS))` 的实际延时
  - 配置位置：`sdkconfig` 中的 `CONFIG_FREERTOS_HZ=1000`
...
### 问题 5: 延时不准确或速度异常
**可能原因**：
- ❌ FreeRTOS Tick 频率配置错误
- ❌ 系统时钟频率不是 1000Hz

**解决方法**：
1. 检查 `sdkconfig` 文件中的配置：
   ```
   CONFIG_FREERTOS_HZ=1000
   ```
2. 如果是其他频率（如 100Hz），需要调整延时计算：
   - 100Hz → 每步延时应设为 20ms（而不是 2ms）
   - 500Hz → 每步延时应设为 4ms（而不是 2ms）
3. 推荐使用 1000Hz 以获得最佳精度和性能
